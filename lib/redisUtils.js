...
var redisUtil = {
  ...
  delx: function (key, cb, options) {
    ...
    //Hash Operations
  hsetx: function (hkeys, keys, data, ttl, cb, options) {
    var value = JSON.stringify(data);
    var selectDB = options && Number(options.selectDB) ? Number(options.selectDB) : 0;
    redis.select(selectDB, function (e, r) {
      if (e) {
      ...
      redis.hsetex(putils.getRedisKey(hkeys), putils.getRedisKey(keys), ttl, value, cb);
    });
  },

  hkeys: function (hkeys, cb, options) {
    var selectDB = options && Number(options.selectDB) ? Number(options.selectDB) : 0;
    redis.select(selectDB, function (e, r) {
      if (e) {
        return cb(e);
      }
      redis.hkeys(putils.getRedisKey(hkeys), cb);
    });
  },

  hgetx: function (hkeys, keys, cb, options) {
    var hkey = putils.getRedisKey(hkeys);
    var key = putils.getRedisKey(keys);
    var selectDB = options && Number(options.selectDB) ? Number(options.selectDB) : 0;
    redis.select(selectDB, function (e, r) {
      if (e) {
        return cb(e);
      }
      redis.hget(hkey, key, respond);
    });
    ...
    function respond(err, data) {
      ...
      if (!err && data) {
        ...
        err = err || new Error('No search results found in redis for field: ' + hkey +
          " and key :" + key);
      }
